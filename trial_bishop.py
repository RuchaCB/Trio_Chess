...
def bishop_validate(piece, initial_pos, desti):
    ...
    for key,value in for_bishop.items():
        if (ini_pos_matrix in key) and (desti_pos_matrix in key):
            if value[0].index(initial_pos[0]) - value[1].index(initial_pos[1:]) == value[0].index(desti[0]) - value[1].index(desti[1:]):
                return True
    return False
...
def king_validate(piece, initial_pos, desti):
    ...
    if initial_pos[0] == desti[0] and (str(int(initial_pos[1:])+1) == desti[1:] or str(int(initial_pos[1:])-1) == desti[1:]):# C3 -> C2 or C3 -> C4
        ...
        return True
    elif (initial_pos[1] in extreme_column) and (desti[1] in extreme_column) and ((ord(initial_pos[0])==ord(desti[0]))or(ord(initial_pos[0])+1==int(desti[0]))or(ord(initial_pos[0])-1==int(desti[0]))):
        return True
    else:
        return False
...
def move(p, desti):
    ...
    if p[1]=="P":
        ...
        if initial_pos!= desti and pawn_validate(star['P'][p][0], initial_pos, desti):
            capture(desti)
