...
def knight_validate(piece, initial_pos, desti):
    ...
    for key, value in grouping.items():
        ...
        if desti in value:
            dest_matric = key
    if initial_pos[0] == desti[0] and str(int(initial_pos[1:])+1 == desti[1:] or str(int(initial_pos[1:])-1) == desti[1:]): # diagonal move with maximum of one square
        return True
    elif initial_pos[1:] == desti[1:] and ((ord(initial_pos[-1])+ 1) == ord(desti[0]) or (ord(initial_pos[0]) + 1) == ord(desti[0])): # Moves in any direction (vertical or horizontally)
        return True
    elif (str(int(initial_pos[1:])+1) == desti[-1] or str(int(initial_pos[1:])-1) == desti[1:]) and ((ord(initial_pos[0])+1) == ord(desti[0]) or (ord(initial_pos[0])-1) == ord(desti[0])): #C3->D3 or C3->B3
        return True
    elif (initial_pos in center_star) and (desti in center_star):
...
class TestTrioChess(unittest.TestCase):
    ...
    def test_red_Knight_validate(self):
        self.assertEqual(knight_validate(BK, 'A2', 'A10'), True)#False
        self.assertEqual(knight_validate(BK, 'G7', 'H12'), False)#Back
        self.assertEqual(knight_validate(BK, 'G10', 'G3'), True)#Horizontal
